...
export const createAttributeSchema = z.object({
  name: z
    ...
    .max(100, 'Attribute name too long'),
  values: z
...
export const updateVariantSchema = z.object({
  sku: z.string().min(1).max(100).regex(/^[A-Za-z0-9\-_]+$/).optional(),
...
export const productQuerySchema = z.object({
  ...
  hasVariant: z.enum(['true', 'false']).optional().transform(v => v === 'true'),
  inStock: z.enum(['true', 'false']).optional().transform(v => v === 'true'),
  minPrice: z.string().regex(/^\d+$/).optional().transform(v => v ? parseInt(v) : undefined),
  maxPrice: z.string().regex(/^\d+$/).optional().transform(v => v ? parseInt(v) : undefined),
  includeVariants: z.enum(['true', 'false']).optional().transform(v => v === 'true'),
  limit: z.string().regex(/^\d+$/).optional().transform(v => v ? parseInt(v) : 10),
  offset: z.string().regex(/^\d+$/).optional().transform(v => v ? parseInt(v) : 0),
});
